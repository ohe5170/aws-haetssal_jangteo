...
public class AdminAPIController {
    ...
    public HomeWithPagingDTO getHomeData() {
        log.info("관리자 홈 데이터 조회");
        return adminService.getHomeData();
    }

    @GetMapping("reports/{page}")
    public ReportWithPagingDTO getReports(@PathVariable int page, Search search) {
        log.info("관리자 신고 목록 조회 - 페이지: {}, 검색조건: {}", page, search);
        return adminService.reportList(page, search);
    }

    @PutMapping("reports/{id}")
    public void updateReportState(@PathVariable Long id, @RequestBody ReportDTO reportDTO) {
        log.info("신고 상태 수정 - ID: {}, 상태: {}", id, reportDTO);
        adminService.updateReportState(id, reportDTO.getReportType().name());
    }
}
