...
public class AdminService {
    ...
    private final AdminHomeDAO adminHomeDAO;
    ...
    // 신고 목록 조회
    public ReportWithPagingDTO reportList(int page, Search search) {
        ReportWithPagingDTO reportWithPagingDTO = new ReportWithPagingDTO();
        int total = adminReportDAO.findTotal(search);
        Criteria criteria = new Criteria(page, total);

        List<ReportDTO> reports = adminReportDAO.findAll(criteria, search);

        criteria.setHasMore(reports.size() > criteria.getRowCount());
        reportWithPagingDTO.setTotal(total);
        reportWithPagingDTO.setCriteria(criteria);

        if (criteria.isHasMore()) {
            reports.remove(reports.size() - 1);
        }
        reportWithPagingDTO.setReports(reports);

        return reportWithPagingDTO;
    }
    ...
    public HomeWithPagingDTO getHomeData() {
        ...
        List<ItemDTO> items = adminHomeDAO.selectItemAll();
        List<StoreDTO> stores = adminHomeDAO.findAllStores();
        List<UserDTO> users = adminHomeDAO.findUserAll();

        homeWithPagingDTO.setItems(items);
        homeWithPagingDTO.setStores(stores);
        homeWithPagingDTO.setUsers(users);
        homeWithPagingDTO.setTodaySalesCount(adminHomeDAO.getTodaySalesCount());
        homeWithPagingDTO.setTotalUserCount(adminHomeDAO.getTotalUserCount());
        homeWithPagingDTO.setTodayReportCount(adminHomeDAO.getTodayReportCount());
